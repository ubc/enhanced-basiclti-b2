...
<%!
  private static String getIParam(String paramName, Integer iValue, String suffix) {

    String param = "";
    if (iValue != null) {
      param = " " + paramName + "=\"" + String.valueOf(iValue) + suffix + "\"";
    }
...
<%
  ...
  if (ltiLog.length() > 0) {
    B2Context.log(false, "LTI log: " + ltiLog);
  }
  ltiLog = b2Context.getRequestParameter(Constants.LTI_ERROR_LOG, "");
  if (ltiLog.length() > 0) {
    B2Context.log(true, "LTI error log: " + ltiLog);
  }
    ...
    for (Iterator<ContentItem> iter = resp.getContentItems().iterator(); iter.hasNext();) {
      ...
      if (thumbnail != null) {
        ...
        thumbnailHtml = "<img src=\"" + thumbnail.getId() +
           ...
           getIParam("width", thumbnail.getWidth(), "px") +
           getIParam("height", thumbnail.getHeight(), "px") +
           " style=\"display: inline; float: left; border: 0; padding-right: 10px;" + maxWidth + "\" />";
        if (url.length() > 0) {
